...
// Assuming these are generated by ANTLR4
...
private:
  ...
  // Helper to get a printf function for debugging/output
...
public:
  ...
  // Initialize external functions like printf, sin, cos, etc.
  void setupExternalFunctions() {
    // Printf setup
    // PointerType::get is deprecated, use getUnqual for opaque pointers or
    // get(Type*, AddrSpace)
  ...
  // Helper to promote integer to double if needed
  ...
  virtual std::any visitProg(CalcFisParser::ProgContext *ctx) override {
    ...
    // Setup format strings for printf inside main where we have an insertion
    // point
    // Fixed: CreateGlobalStringPtr -> CreateGlobalString
    // Detección de nombre de la variable
    /*string fmt_d = varName + ": %d\n";
    string fmt_f = varName + ": %f\n";
    //formatStrInt = builder->CreateGlobalString("Result: %d\n", "fmt_int");
    // Aplicando el builder con nombre de variable para reconocimiento práctico
    formatStrInt = builder->CreateGlobalString(fmt_d, "fmt_int" + varName);
    //formatStrFloat = builder->CreateGlobalString("Result: %f\n", "fmt_float");
    formatStrFloat = builder->CreateGlobalString(fmt_f, "fmt_float" + varName);*/
  ...
  visitAssignStat(CalcFisParser::AssignStatContext *ctx) override {
    ...
    // Optional: Print assignment result
  ...
  visitAddSubMatExpr(CalcFisParser::AddSubMatExprContext *ctx) override {
    ...
    // Fixed: Check operator using token existence
  ...
  visitMulDivMatExpr(CalcFisParser::MulDivMatExprContext *ctx) override {
    ...
    // Note: Grammar doesn't seem to have // operator in matExpr rule shown,
    // but logic handles it if it were there. Assuming only * and / for now
    // based on grammar.
  ...
  visitModMatExpr(CalcFisParser::ModMatExprContext *ctx) override {
    ...
    if (!L || !R)
      return (Value *)nullptr;
    // Modulo usually implies integers
  ...
  visitPowerMatExpr(CalcFisParser::PowerMatExprContext *ctx) override {
    ...
    // pow(L, R)
    // Fixed: getDeclaration -> getOrInsertDeclaration
  ...
  Value *createUnaryMathCall(std::string name, Value *arg) {
    arg = castToDouble(arg);
    // We can use intrinsics for sin/cos, or C library calls for others
    if (name == "sin") {
    ...
    // For tan, etc., we might need to declare them as external functions if
    // intrinsics don't exist or implement them using sin/cos. LLVM doesn't have
    // a 'tan' intrinsic, so we use the C library function.
  ...
  // Boilerplate for other visitors to avoid warnings/errors if they are called
  ...
  virtual std::any visitOperPars(CalcFisParser::OperParsContext *ctx) override {
    ...
    // Fixed: Cast to ParParContext to access par()
    // The rule is: pars : par (',' par)* #ParPar
    // So ctx->pars() returns ParsContext*, which we cast to ParParContext*
    if (auto parParCtx =
  ...
  // Helper to print result with label
  void printResult(const std::string &label, Value *val,
                   const std::string &unit = "") {
    // Create format string: "label: %f unit\n"
  ...
  std::any operCaidaLibre(std::map<std::string, Value *> &pars) {
    ...
    Value *vf = pars["vf"];
    Value *t = pars["t"];
    Value *a = pars["a"]; // Optional, default 9.81
